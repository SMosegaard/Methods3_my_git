...
token_d <- read.csv("token_train.csv")
demo_d <- read.csv("demo_train.csv")
LU_d <- read.csv("LU_train.csv")
...
token_d <- token_d %>% 
  ...
  mutate(Visit = recode(Visit, 
        visit1.  = 1, Visit1. = 1, visit2. = 2, Visit2. = 2, Visit3.= 3, visit3. = 3,
        visit4. = 4, Visit4. = 4, visit5. = 5, Visit5. = 5, visit6. = 6,  Visit6. = 6))
...
merged_clean_data <- merge(demo_clean, LU_clean, all = T)

merged_clean_data <- merge(merged_clean_data, token_clean, all = T)
...
data_visit1 <- merged_clean_data %>% 
  filter(Visit=="1") %>% 
  select(ADOS, ID, MullenRaw, ExpressiveLangRaw, Socialization) %>% 
  rename(ADOS1=ADOS) %>% 
  rename(MullenRaw1=MullenRaw) %>% 
  rename(ExpressiveLangRaw1=ExpressiveLangRaw) %>% 
  rename(Socialization1 = Socialization)

final_data <- merge(data_visit1, merged_clean_data, all = T)
...
final_data <- final_data %>% 
  mutate(Gender = str_replace_all(Gender, '1', 'Male')) %>% 
  mutate(Gender = str_replace_all(Gender, '2', 'Female'))

final_data <- final_data %>% 
  mutate(Diagnosis = str_replace_all(Diagnosis, 'A', 'ASD')) %>% 
  mutate(Diagnosis = str_replace_all(Diagnosis, 'B', 'TD'))
```


```{r}
write.csv(final_data, file='final_data_class1.csv')
```
